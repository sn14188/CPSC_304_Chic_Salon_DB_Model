...
<h2>Display Information</h2>
    <form method="GET" action="AllServices.php">
    ...
    {
        echo "<br>Retrieved data from table Service:<br>";
        echo "<table>";
        echo "<tr><th>serviceID</th><th>price</th><th>duration</th></tr>";
        while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
            echo "<tr><td>" . $row["SERVICEID"] . "</td><td>" . $row["PRICE"] . "</td><td>" . $row["DURATION"] . "</td></tr>";
        }
        echo "</table>";
    }
    ...
    function handleDeleteCascadeTupleRequest()
    {
        global $db_conn;

        $r_ID = $_POST['insDrId'];

        executePlainSQL("DELETE FROM Service WHERE serviceID ='" . $r_ID . "'");
    ...
    function handleGETRequest()
    {
        if (connectToDB()) {
            if (array_key_exists('countTuples', $_GET)) {
                ...
                handlePrintTableRequest('Service');
    ...
    <h3>Cascade Delete Data from Service - will remove entry in Color/Style/Haircut tables</h3>
    <form method="POST" action="AllServices.php">
        <input type="hidden" id="deleteCascadeTupleRequest" name="deleteCascadeTupleRequest">
        serviceID: <input type="text" name="insDrId"> <br /><br />
